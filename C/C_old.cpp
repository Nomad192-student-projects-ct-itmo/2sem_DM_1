...
public:
    ...
    n_type n_words() const
    {
        ...
        bool *was_theres = new bool[n + 1]{true, true};
        for(size_t i = 2; i < n + 1; i++)
            was_theres[i] = false;

//        printf("was there: ");
//        for(size_t i = 0; i < n + 1; i++)
//            printf("%d ", was_theres[i]);
//        printf("\n");

        while(!cur_states->empty())
        {
            auto *new_cur_states = new map<size_t, size_t>;
            auto *new_was_theres = new set<size_t>;
            ...
            for(auto cur_state : *cur_states) {
                for (int i = 0; i < Alphabet_size; i++) {
                    ...
                    if(new_state != 0) {
                        if(was_theres[new_state]) {delete[] was_theres; delete new_was_theres; delete cur_states; return -1;}
                        new_was_theres->insert(new_state);
                        (*new_cur_states)[new_state] += cur_state.second;
            ...
            for(auto new_was_there : *new_was_theres) {
                was_theres[new_was_there] = true;
            }
            delete new_was_theres;
